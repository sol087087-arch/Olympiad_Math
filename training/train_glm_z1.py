...
# Force single-process mode so accelerate doesn't fight with device_map='auto'
...
# ===================== CONFIG =====================
RESUME_FROM = None
BASE_MODEL = r"C:\models\GLM-Z1-9B-0414"
DATASET_PATH = r"C:\lora_training\OLympiad\converted_for_sft.jsonl"

MAX_SEQ_LENGTH = 8192
DTYPE = None
LOAD_IN_4BIT = True  # 9B model is ~18GB in bf16, doesn't fit in 16GB VRAM
...
if RESUME_FROM:
    ...
    OUTPUT_DIR = f"C:/lora_training/lora_MATH_output/glm_run_{timestamp}"
    Path(OUTPUT_DIR).mkdir(parents=True, exist_ok=True)
    print(f"NEW RUN -> {OUTPUT_DIR}\n")

# ===================== FORMAT CONVERSION =====================
# Dataset is in ChatML format; GLM-Z1 uses its own special tokens.
# We convert on the fly so the model sees its native format.

def chatml_to_glm(text: str) -> str:
...
dataset = dataset.map(lambda ex: {"text": chatml_to_glm(ex["text"])})

print("\nFirst example after conversion (first 600 chars):")
print(repr(dataset[0]["text"][:600]), "\n")
...
# GLM already has pad_token = <|endoftext|>, but just in case:
...
# ===================== COMPLETION MASKING =====================
# We mask everything up to and including "Solution:\n",
# so only the actual solution text contributes to the loss.
...
def tokenize_and_mask(example):
    ...
    # Find the LAST occurrence of response_template (in case it appears in the problem too)
    start_idx = None
...
if num_train_tokens == 0:
    raise ValueError(
        "Masking failed: 0 trainable tokens in first sample.\n"
        "Check that the converted text contains the string 'Solution:\\n'."
    )
...
# Pad sequences in each batch; labels=-100 stays masked
